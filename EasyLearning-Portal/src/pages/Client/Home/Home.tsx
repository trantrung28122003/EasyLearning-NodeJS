...
const Home: React.FC = () => {
  ...
  const [coursesWithFree, setCoursesWithFree] = useState<Course[]>([]);
  ...
  const doGetCoursesWithFree = () => {
    DoCallAPIWithOutToken(GET_COURSES_WITH_FREE, "get").then((res) => {
      if (res.status === HTTP_OK) {
        const data: ApplicationResponse<Course[]> = res.data;
        setCoursesWithFree(data.result);
      }
    });
  };
          ...
          <div className="container-xxl py-5">
            <div className="container">
              <div className="text-center wow fadeInUp" data-wow-delay="0.1s">
                ...
                <h1 className="mb-5">Các Khóa Học Miễn Phí Dành Cho Bạn</h1>
              </div>
              <div className="row g-4 justify-content-center">
                {coursesWithFree.map((course, index: number) => {
